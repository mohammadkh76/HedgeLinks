angModule.controller('addBottomSliderController',['$scope','dataService','convertorService','$window','Upload',function ($scope,dataService,convertorService,$window,Upload) {
    $scope.changeUploadedPic = function () {
        if ($scope.data.File){
            $scope.showLoading=true;
            convertorService.toBase64($scope.data.File).then(function(res){
                $scope.uploadPreview=res.base64;
                $scope.showLoading=false;
                $scope.$apply();

            });

        }
    }
    $scope.progressPercentage=0;
    $scope.showLoading=0;
    $scope.submit = function (data) {
        dataService.upload({
            url: '/api/BottomSliderApi/AddBottomSlider/',


            data: {
                Title: $scope.data.Title,
                Description: $scope.data.Description,
                File: $scope.data.File,
            },
            successCb(res) {
                $window.location.href = "/BottomSliders/Index";
            },
            failureCb(res){
                $scope.message=res.data;

            },
            progressCb(evt) {
                $scope.progressPercentage = parseInt(100.0 * evt.loaded / evt.total);
                console.log('progress: ' + $scope.progressPercentage + '% ' + evt.config.data);
            },
           
        }).catch(function (err) {
            $scope.errorMessage = err.data;
        })
    };

 


}]);